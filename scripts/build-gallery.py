...
def find_image(folder: Path, stem: str) -> Path | None:
    """Find epXX image; prefer exact stem match, any image extension."""
    matches = []
...
def scan_chapter(folder: Path) -> dict | None:
    ...
    for n in sorted(episodes):
        ...
        if not note:
            note_path = folder / f"ep{n:02d}.txt"
            if not note_path.exists():
                # also try ep1.txt style
                note_path = folder / f"ep{n}.txt"
            note = read_text(note_path) if note_path.exists() else ""

        # Path relative to repo root, forward slashes for web
